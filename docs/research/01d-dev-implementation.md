...
- **4f.4 (Post-deploy Hotfix Cycle #2):** Shilo's 2nd post-deploy smoke (after `bda01c1`) surfaced 3 new issues. Investigation in `docs/research/01f-hotfix-investigation.md` (cd24a28). Branch `hotfix/4f4-slash-dates-pubmed-quality` off `bda01c1`. **ALL 3 RESOLVED:** Commit A (Issue #2 dates — adapter-only) `452fe4b`; Commit B (Issue #3 PubMed quality — adapter-only + Supabase MCP DB cleanup per Q5: 14 irrelevant pubmed rows deleted) `53e1087`; Commit C (Issue #1 /research slash — bot/telegram.js +~6 LOC, regression FROM 4f.3 Commit B) `b6bcf26`. Tests 190 → 206/206 PASS. `bot/*` delta from this cycle: telegram.js +~6 LOC for slash handler only.
- **4f.4.M (Merge):** `--no-ff` merge of `hotfix/4f4-slash-dates-pubmed-quality` → `main` (`af9f51a`). All 3 issues resolved (`452fe4b`, `53e1087`, `b6bcf26`). 206/206 tests. `bot/*` delta: telegram.js +~6 LOC. Triggers Render auto-redeploy. Final verification gated on Shilo's RT01/RT02/RT03 retest.
